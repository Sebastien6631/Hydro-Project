...
| 4.2 | **Détection de dérive Evidently** | xh | ✅ | PR #16 mergée `dev`+`main`. `monitoring/drift.py` (test K-S par colonne, seuil 40% de colonnes en dérive = `dataset_drift`), `cron/scripts/check-drift.py` (jamais bloquant, contrairement à `validate-data.py`). Rapports JSON sous `logs/drift/`, relus (pas recalculés) par `/metrics` (`data_drift_share`, `data_drift_detected`) + alerte `DataDrift`. evidently==0.7.23 (version résolue et testée en vrai avant d'écrire le code). |
| 4.3 | **Mises à jour automatisées du modèle** | sg | ✅ | Couvert par `airflow/dags/hydro_train.py` (3.1, Sébastien) : DAG hebdo (lundi 02:00) `data_preparation → validate --strict → train --promote`. `train.py` décide seul s'il y a du travail (`is_eligible_for_training`, `RETRAIN_INTERVAL_DAYS=7`) -- règle testée dans le code, pas dupliquée dans le DAG. `--promote` commite/tague en local si le KGE bat la production ; le `git push`/`dvc push` restent volontairement humains (décision documentée, pas un gap). |
| 4.4 | Déploiement cloud (documenté a minima) | xh | ✅ | `phase4/cloud-deployment` → PR vers `dev`. Section README dédiée : tableau service compose → équivalent managé (conteneurs, load balancer+TLS géré, stockage objet, DB, Airflow managé, chart Helm déjà prêt pour un vrai cluster, secrets managés, job `deploy` en CI). Zéro changement de code nécessaire (mêmes images, variables d'env) -- pas de crédits cloud donc rien déployé en continu, même raisonnement que BentoML (3.4) : ne pas payer/coder pour rien. |
